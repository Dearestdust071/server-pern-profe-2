...
describe('POST /api/users', () => {
  ...
  it('retorna 500 si el email ya existe', async () => {
    ...
    expect(res.statusCode).toBe(500); // Sequelize lanza error por unique
  });
});

describe('GET /api/users', () => {
  it('retorna 200 y lista de usuarios', async () => {
...
describe('PUT /api/users/:id', () => {
  ...
  it('actualiza datos válidos', async () => {
    ...
    const res = await request(server).put(`/api/users/${create.body.data.id}`).send({
